...
11. Vercel: from main/, run vercel link --yes (a new project named after the folder), then vercel git connect so every push to main deploys; if it asks to install the Vercel app on GitHub, wait for me. Convex production needs a deploy key: ask me to create one in the Convex dashboard (project settings, deploy keys, production) and paste it here; add it with vercel env add CONVEX_DEPLOY_KEY production, reading the value from what I paste; never write it to a file.
