...
|---|---|---|
| Next.js · Tailwind · Convex | a web app with a database and a password-protected admin page where the status of the project is visible; the stack vibecoding.ru runs on | on your machine only |
...
4. Page /admin: the glass. It renders docs/README.md and docs/journal.md from the files as HTML (any small markdown library is fine), lists every decision (journal headings that carry ⚖️) with its date and name, and shows the result of the last check run from the file the check writes. Reading the files at request time is enough; no database is needed for this page. The page is behind a browser password prompt (HTTP basic auth in Next's request-level check, the middleware or proxy file, whatever the current version calls it): any user name, the password from the ADMIN_PASSWORD variable; without the variable the page answers "closed: set ADMIN_PASSWORD" instead of the content. Generate the password yourself (openssl rand -hex 16), write it into .env.local as ADMIN_PASSWORD (never committed) and list the name in .env.local.example; never print the value.
5. One test: /admin without the password answers 401, and with the password from .env.local lists at least the first decision inside the decisions list (give that list a test id and look only inside it). Make the check green.
...
Done when http://localhost:3000 (or the port you chose, if 3000 is taken) answers with the project page, /admin with the password from .env.local shows the journal with the first decision, the check is green, no placeholders in square brackets are left in main/AGENTS.md, main/docs/README.md and main/docs/journal.md (links do not count), _wt/ exists and the two pointer files are here. Report the two addresses, the check result, the commit, where the admin password lives (the file and the line, never the value), and the one line about Convex.
```
...
2. Add Prettier and Vitest. Add one script "check" that runs the formatter check, the linter (call eslint directly; next lint is gone), the type check and the tests, in that order, and writes its result (green or red, with the time) to a small ignored file, so the admin page can show it. The type check needs Next's generated route types, so generate them inside the check before the type step.
3. Page /: the project name from docs/README.md, one sentence about what it is, and a link to /admin. Plain, readable, no template art.
4. Page /admin: the glass. It renders docs/README.md and docs/journal.md from the files as HTML (any small markdown library is fine), lists every decision (journal headings that carry ⚖️) with its date and name, shows the result of the last check run from the file the check writes, and shows the commit hash and the production address when Vercel's build variables provide them (VERCEL_GIT_COMMIT_SHA, VERCEL_PROJECT_PRODUCTION_URL), "local" otherwise. Reading the files at request time is enough; no database is needed for this page. The page is behind a browser password prompt (HTTP basic auth in Next's request-level check, the middleware or proxy file, whatever the current version calls it): any user name, the password from the ADMIN_PASSWORD variable; without the variable the page answers "closed: set ADMIN_PASSWORD" instead of the content. Generate the password yourself (openssl rand -hex 16), write it into .env.local as ADMIN_PASSWORD (never committed) and list the name in .env.local.example; never print the value.
5. One test: /admin without the password answers 401, and with the password from .env.local lists at least the first decision inside the decisions list (give that list a test id and look only inside it). Make the check green.
6. Install Convex. Add the schema file with no tables yet, a client provider that renders the app without Convex while its URL variable is empty, and an .env.local.example that names the variables the Convex CLI will fill (CONVEX_DEPLOYMENT and NEXT_PUBLIC_CONVEX_URL); make sure .gitignore does not hide the example file. Then run npx convex dev --once --configure new --dev-deployment cloud: it logs me in, creates the dev deployment in the cloud (not a local backend) and writes .env.local, which is never committed. Do not write server functions yet.
7. Fill in the Stack section of main/AGENTS.md: what it is made of, the check command, the run command with the address. Bring docs/README.md in line with what was built.
...
10. From here on, work in one worktree under _wt/ on a branch <agent>-<date>-go-online and finish with one pull request. In it: .github/workflows/check.yml that installs pnpm and runs the check command on every pull request and on every push to main; vercel.json with a build command that depends on the environment (the VERCEL_ENV variable): in production "pnpm check && npx convex deploy --cmd 'pnpm build'", so the check writes its result for /admin and Convex deploys to production and sets the database URL for the build; in preview plain "pnpm build", the app renders without Convex there; docs/deploy.md, the canon of the way out: repository, the check, Vercel, Convex, where the keys live, how to roll back; .github/workflows/prod-alive.yml that requests the address in the repository variable PROD_URL every hour and fails loudly when it does not answer 200; a Deploy line in the Stack section ("push to main → Vercel"); and a second journal entry above the first, as a decision: the project is online, what was created where, what the human does next.
11. Vercel: from main/, run vercel link --yes --project <name of the project folder> (not the folder main/); it connects the repository too, so vercel git connect should answer that it is already connected; if either asks to install the Vercel app on GitHub, give me the link it printed and wait. Then create the Convex production deployment yourself: npx convex deploy from main/ under my login creates it on the first run. Put the admin password into Vercel yourself: read ADMIN_PASSWORD from main/.env.local and pipe it into vercel env add ADMIN_PASSWORD production and vercel env add ADMIN_PASSWORD preview, never printing the value. Production builds on Vercel need a deploy key, and a key never passes through this chat: I put it into Vercel myself. Get the two addresses first: the production dashboard with npx convex dashboard --prod --no-open, and the project's environment variables page in Vercel, https://vercel.com/<team>/<project>/settings/environment-variables (vercel project ls shows the team and the project). Then stop with exactly this, nothing above it: "Create the production deploy key and put it into Vercel; do not paste it here. 1. <Convex link> → Settings → block Deploy Keys → button Create Deploy Key → Name: vercel-production → Expiration: No expiration → tick exactly three permissions: deployment:deploy, deployment:env:view, deployment:env:write → Create → Copy (the key is shown once). 2. <Vercel link> → button Add Environment Variable → Type: Secret → Key: CONVEX_DEPLOY_KEY → Value: paste → Environments: Production only → Save → if Vercel offers Redeploy, press Dismiss: the merge will deploy. Write 'done'." After my "done", check with vercel env ls that CONVEX_DEPLOY_KEY exists in production; never ask for or print its value.
12. Open the pull request, wait for the check, merge it (I said so by pasting this prompt), fast-forward main in main/, remove the worktree and the branch. The merge deploys. Wait for the deployment, take the production address, set it as the repository variable PROD_URL (gh variable set PROD_URL), and request the address once yourself. List the production deployments with vercel ls --prod: every one in Error, including the first one before the key, goes into the report by name.
13. Report.
...
Done when http://localhost:3000 (or the port you chose, if 3000 is taken) answers with the project page locally, the repository exists with main pushed, the pull request's check ran green and the pull request is merged, the production address answers 200 with the project page and its /admin, with the password, shows the journal with two decisions, no placeholders in square brackets are left in main/AGENTS.md, main/docs/README.md and main/docs/journal.md (links do not count), _wt/ holds no work folders again (macOS files like .DS_Store do not count) and the two pointer files are here. Report the repository address, the production address, the check run, the commit, where the admin password lives (the file and the line, never the value), and what remains for me.
```
...
**Why it works in any agent, and with several at once.** The rules are one file every coding agent already reads, `AGENTS.md`; `CLAUDE.md` only points to it. Codex, Claude Code, Cursor, Copilot, Gemini CLI and the next one read the same rules and build the same layout. Several agents can work on one repository at the same time: every task lives in its own worktree under `_wt/`, `main/` stays a clean mirror, and changes meet only through pull requests with the check. Run two agents on two tasks in one afternoon; the layout is built for it.
