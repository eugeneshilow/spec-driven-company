...
Un tercer archivo es opcional: `refs.md`, el registro de referencias de las que una zona tomó sus moldes. Aparece con la primera referencia y nunca se crea vacío. Una línea por referencia, `[nombre](dirección) — qué tomamos de ella`, agrupadas por lo que el donante hace mejor, con una nota corta al final sobre qué tienen en común las fuertes. Las referencias nuevas se añaden al final; una carpeta nunca tiene un segundo archivo de referencias.
...
3. **Especificación antes que código.** Requisitos, plan y diseño en un solo documento antes de la primera línea de código. Para cada forma que contenga, una página, un documento, una API, un nombre, busca quién resolvió mejor la misma tarea y toma su molde. Busca dos veces, en este orden: primero los líderes de tu propio campo, qué usan y si existe una forma asentada; después el mundo ancho, otras industrias y la vida fuera del software, porque el mejor molde suele ser un mecanismo de otro sitio, y esa segunda pasada es de donde sale una transferencia original. Busca al donante por la tarea que resuelve, no por el tema del que trata. Inventa desde cero solo cuando puedas decir por qué ningún molde encaja. Cada referencia encontrada va a `refs.md` en el mismo cambio y a la persona como lista en la respuesta; una referencia que se queda en el chat es trabajo perdido. El plan son pasos ordenados, no fechas: un paso está hecho cuando están hechos los pasos de los que depende.
